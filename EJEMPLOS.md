# Notas de los ejemplos — ruta /ejemplos

Material de apoyo para cada criterio WCAG mostrado en `/ejemplos` (slide conceptual + demo Bad/Good, cuando existe). Acá vive el detalle completo — qué está mal, la solución, y cualquier instrucción de puesta en escena — que ya no forma parte de `GUION.md`: en la charla en vivo (`/presentacion`) estos 55 criterios se mencionan brevemente, sin detenerse en el demo.

El orden sigue exactamente el de `/ejemplos`. Los `<!-- id:X -->` coinciden con los `topicId` usados en `src/components/Examples.jsx`.
...
El contenido debe funcionar tanto en orientación vertical como horizontal, salvo que una orientación sea esencial para la función (por ejemplo, una aplicación de piano). Bloquear la orientación sin justificación excluye a quien usa el dispositivo en un soporte fijo. Este criterio se explica sin demostración interactiva — no tiene par Bad/Good.
...
Ningún contenido puede destellar más de tres veces por segundo: puede inducir convulsiones en personas con epilepsia fotosensible. Este criterio se explica sin demostración en vivo, por razones de seguridad — no tiene par Bad/Good.
...
Las funciones activadas por el movimiento del dispositivo (agitar, inclinar) deben tener un control equivalente en pantalla y poder desactivarse. Este criterio se explica sin demostración, dado que requiere un dispositivo físico móvil — no tiene par Bad/Good.
...
El idioma principal del documento debe declararse mediante el atributo `lang`. Sin esa declaración, un lector de pantalla puede aplicar reglas fonéticas de otro idioma a todo el contenido. Este criterio se explica a nivel de documento, sin demostración interactiva — no tiene par Bad/Good.
