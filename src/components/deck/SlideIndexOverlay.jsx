  ...
  return (
    <div
      ...
      <div
        ...
        className="flex max-h-[70vh] w-full max-w-xl flex-col overflow-hidden rounded-xl border border-deck-ink/10 bg-deck-panel shadow-2xl">
        <div className="flex items-center gap-2 border-b border-deck-ink/10 px-4 py-3">
          <Search className="h-4 w-4 shrink-0 text-deck-ink/40" />
          <input
            ref={inputRef}
            value={query}
            onChange={(event) => {
            ...
            className="w-full bg-transparent text-sm text-deck-ink placeholder:text-deck-ink/40 focus:outline-none"
          />
        </div>
        <div className="overflow-y-auto py-1">
          {filtered.length === 0 && (
            <p className="px-4 py-6 text-center text-sm text-deck-ink/40">Sin resultados.</p>
          )}
          {filtered.map((entry, row) => (
            <button
              ...
              className={`flex w-full items-center justify-between gap-3 px-4 py-2 text-left text-sm transition-colors ${
                row === highlighted ? 'bg-deck-ink/10' : ''
              }`}>
              <span className="flex min-w-0 items-center gap-3">
                <span className="w-8 shrink-0 text-right text-xs tabular-nums text-deck-ink/30">
                  {entry.index + 1}
                </span>
                <span
                  className={`min-w-0 flex-1 truncate ${
                    entry.index === currentIndex ? 'font-semibold text-deck-ink' : 'text-deck-ink/70'
                  }`}>
                  {entry.label}
                </span>
                {entry.wcag && (
                  <span className="shrink-0 rounded-full bg-deck-teal/15 px-2 py-0.5 text-[10px] font-medium tabular-nums text-deck-teal">
                    {entry.wcag}
                  </span>
                )}
              </span>
              {entry.index === currentIndex && (
                <span className="shrink-0 rounded-full bg-deck-ink/10 px-2 py-0.5 text-[10px] uppercase tracking-wide text-deck-ink/50">
                  Actual
                </span>
              )}
            </button>
        ...
        <div className="border-t border-deck-ink/10 px-4 py-2 text-[11px] text-deck-ink/30">
